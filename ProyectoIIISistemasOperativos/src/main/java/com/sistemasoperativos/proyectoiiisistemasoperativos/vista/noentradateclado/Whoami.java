 ...
 * @author andre
 */
public class Whoami extends ComandoPadre {
    
    public Whoami(String comando, Controlador controlador){
    ...
    public void EjecutarComando(String comando) throws Exception {
        if(ContarTamanoComando(comando) != 0){
            throw new Exception("Whoami esperaba 0 argumentos, en cambio recibió " + ContarTamanoComando(comando) + " argumentos.");
        }
        String respuesta = ControladorAsignado.Whoami();
        System.out.println("\n" + respuesta);
    }
}
