 ...
 * @author andre
 */
public class CloseFile extends ComandoPadre {
    
    private String NombreArchivo;
    
    public CloseFile(String comando, Controlador controlador){
    ...
    public void EjecutarComando(String comando) throws Exception {
        if(ContarTamanoComando(comando) != 1){
            throw new Exception("CloseFile esperaba 1 argumentos, en cambio recibió " + ContarTamanoComando(comando) + " argumentos.");
        }
        ExtraerParametrosActuales(comando);
        String respuesta = ControladorAsignado.CloseFile(NombreArchivo);
