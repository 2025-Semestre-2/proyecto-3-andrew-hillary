 ...
 * @author andre
 */
public class Touch extends ComandoPadre {
    private String DirectorioDestino;
    
    public Touch(String comando, Controlador controlador){
    ...
    public void EjecutarComando(String comando) throws Exception {
        if(ContarTamanoComando(comando) != 1){
            throw new Exception("Touch esperaba 1 argumentos, en cambio recibió " + ContarTamanoComando(comando) + " argumentos.");
        }
        ExtraerParametrosActuales(comando);
        String respuesta = ControladorAsignado.Touch(DirectorioDestino);
