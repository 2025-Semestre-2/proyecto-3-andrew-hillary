 ...
 * @author andre
 */
public class OpenFile extends ComandoPadre {
    
    private String DirectorioDestino;
    
    public OpenFile(String comando, Controlador controlador){
    ...
    public void EjecutarComando(String comando) throws Exception {
        if(ContarTamanoComando(comando) != 1){
            throw new Exception("OpenFile esperaba 1 argumentos, en cambio recibió " + ContarTamanoComando(comando) + " argumentos.");
        }
        ExtraerParametrosActuales(comando);
        String respuesta = ControladorAsignado.OpenFile(DirectorioDestino);
